...
from services.watchlistDelivery.spokeMapping import SPOKE_MAPPING
import sys
import os
...
if __name__ == "__main__":
    with JobLog(job_name="spoke_sync_engine", log_dir=Path("logs"), timezone_name="UTC") as log:
        
        log.event("JOB_STARTED", started_at=log.stamp(), sources="spoke_sync", timezone="UTC")
        
        try:
            # -------------------------------------------------------------
            # CONTROL PANEL: Use the specific operation you want to trigger
            # -------------------------------------------------------------
