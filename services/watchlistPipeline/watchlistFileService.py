...
def build_api_task(config: dict[str, Any]) -> ApiCollectorTask:
    """
    Build the API collector task from a watchlist config's ``api_config`` block.

    Shared by the real acquisition path (``_collect_api_source``) and the
    dry-run planning script, so both construct the task identically -- the plan
    a dry run shows is exactly what a real run would use.
    """
